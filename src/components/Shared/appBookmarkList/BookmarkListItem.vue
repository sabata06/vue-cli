    ...
    <div class="p-3">
      ...
      <div class="flex items-center justify-center mt-2 gap-x-1">
        ...
        <button
          ...
          :class="{
            'like-item-active': alreadyBookmarked
          }"
        >
  ...
  methods: {
    ...
    bookmarkItem() {
      ...
      if (!this.alreadyBookmarked) {
        ...
        bookmarks = bookmarks.filter(bm => bm !== this.item.id)
      }

      this.$appAxios.patch(`/users/${this._getCurrentUser.id}`, { bookmarks }).then(bm_res => {
        console.log(bm_res);
